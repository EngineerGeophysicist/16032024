...
# Преобразование в тензоры PyTorch и перемещение на GPU
X_train = torch.from_numpy(X_train).float().to(device)
y_train = torch.from_numpy(y_train).float().to(device)
X_test = torch.from_numpy(X_test).float().to(device)
y_test = torch.from_numpy(y_test).float().to(device)

# Определение LSTM модели с двумя скрытыми слоями
class LSTM(nn.Module):
    def __init__(self, input_size=1, hidden_layer_size=100, output_size=1):
        super().__init__()
        self.hidden_layer_size = hidden_layer_size
        self.lstm = nn.LSTM(input_size, hidden_layer_size, num_layers=2)  # Добавлено num_layers=2
        self.linear = nn.Linear(hidden_layer_size, output_size)
        self.hidden_cell = (torch.zeros(2,1,self.hidden_layer_size).to(device),  # Изменено на 2, чтобы соответствовать num_layers=2
                            torch.zeros(2,1,self.hidden_layer_size).to(device))  # Изменено на 2, чтобы соответствовать num_layers=2
...
# Обучение LSTM модели на GPU
model = LSTM().to(device)
loss_function = nn.MSELoss()
optimizer = torch.optim.SGD(model.parameters(), lr=0.01)  # Используем SGD вместо Adam
epochs = 150

for i in range(epochs):
    for seq, labels in zip(X_train, y_train):
        optimizer.zero_grad()
        model.hidden_cell = (torch.zeros(2, 1, model.hidden_layer_size).to(device),  # Изменено на 2, чтобы соответствовать num_layers=2
                              torch.zeros(2, 1, model.hidden_layer_size).to(device))  # Изменено на 2, чтобы соответствовать num_layers=2
...
for i in range(len(X_test)):
    with torch.no_grad():
        model.hidden = (torch.zeros(2, 1, model.hidden_layer_size).to(device),  # Изменено на 2, чтобы соответствовать num_layers=2
                         torch.zeros(2, 1, model.hidden_layer_size).to(device))  # Изменено на 2, чтобы соответствовать num_layers=2
